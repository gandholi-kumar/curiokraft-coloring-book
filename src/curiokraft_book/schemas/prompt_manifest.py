...
class PromptItem(BaseModel):
    ...
    id: str = Field(..., description="Unique prompt identifier (e.g. COVER_FRONT, COVER_BACK, P001, P006)")
    page_number: Optional[int] = Field(default=None, description="Page number for interior pages, null for covers")
    label: str = Field(..., description="Descriptive title or object name")
    type: Literal["front_cover", "back_cover", "interior_page"] = Field(..., description="Asset type category")
    section: str = Field(default="General", description="Book section (e.g. Covers, A-Z Alphabet, Fruits & Vegetables)")
    drop_target: str = Field(..., description="Target file path relative to publications root, e.g. inbox/raw_pages/raw_p006_banana.png")
    preset_name: Literal["CurioKraft - Interior Coloring Pages", "CurioKraft - Cover Art Master"] = Field(
        ..., description="Google AI Studio saved System Instructions preset name to select"
    )
    aspect_ratio: str = Field(default="3:4", description="Aspect ratio for AI Studio sidebar")
    output_format: str = Field(default="Images only", description="Output format for AI Studio sidebar")
    temperature: float = Field(..., description="Sampling temperature (0.5 for interior line art, 0.9 for covers)")
    ...
    chat_id: Optional[str] = Field(default=None, description="Captured Google AI Studio session/chat ID from URL")
